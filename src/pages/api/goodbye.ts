...
async function getActivities(location: string, button: string){ //takes in an input from the website url
...
export default async function handler( //calls the function automatically
  ...
  const results = await getActivities(location, button) //getRecs is the function we used earlier to call GBT to give us recommendations
