...
async function getPlace(location: string, button: string) {
  const config: AxiosRequestConfig = {
    ...
    params: {
      ...
      fields: 'place_id,photos', // Include the 'photos' field to get latitude and longitude
    },
  };
  ...
  // Now fetch detailed information for the place using 'place_id' to get latitude and longitude
  const placeDetailsConfig: AxiosRequestConfig = {
    method: 'get',
  ...
  const placeDetailsResponse = await axios(placeDetailsConfig);
  const placeDetailsData = placeDetailsResponse.data;

  if (!placeDetailsData || !placeDetailsData.result || !placeDetailsData.result.geometry) {
    return null;
  }

  const { lat, lng } = placeDetailsData.result.geometry.location;
  return { lat, lng, placeId, photoId };
}
...
export default async function handler(
  ...
  if (!location || Array.isArray(location) || !button || Array.isArray(button)) {
    res.status(400).json({ 'error_code': 'location or button invalid' });
    return;
  }
